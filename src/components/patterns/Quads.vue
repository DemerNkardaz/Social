...
const params = computed(() => {
  ...
  const fluid = (px: number) => (px / 16) * fs

  const cell = fluid(props.sizePx)
  const gap  = fluid(props.gapPx)
  const step = cell + gap

  const rx = typeof props.borderRadius === 'number'
    ? fluid(props.borderRadius)
    : (parseFloat(props.borderRadius as string) / 100) * (cell / 2)

  return { cell, gap, step, rx }
})

// Resolve which cells are filled → Set of "col,row" keys
const filledCells = computed<Set<string>>(() => {
  const set = new Set<string>()
  const { row, col } = props

  if (row) {
    // row-mode: count = number of columns
